...
from RBAC.teams.views import (
    ...
    remove_team_member,
    get_team_members,
    get_team_by_id, get_org_members
)
...
router.add_api_route("/", endpoint=get_teams_by_org, methods=["GET"], description="Get all teams in the org")
router.add_api_route("/{team_id}", endpoint=get_team_by_id, methods=["GET"], description="Get a team by ID", dependencies=[Depends(get_user_data_from_request)])
...
router.add_api_route("/members", endpoint=get_org_members, methods=["GET"], description="Get all members of an org")